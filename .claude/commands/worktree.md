...
**IMPORTANT**: The main repo and worktrees have different docker-compose.yml configurations:

- **Main repo**: Full docker-compose.yml with `redis-celery` and `redis-cache` services (for
  production-like setup)
- **Worktrees**: Simplified docker-compose.yml with a single `redis` service (auto-generated by
  worktree.sh)

The `dev-worktree.sh` script automatically detects which configuration is present and starts the
appropriate Redis services. **Do NOT commit a worktree's docker-compose.yml to the main repo** - it
will break the main development setup.
