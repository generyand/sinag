...
function extractTagsFromOpenAPI(openAPISpec) {
  ...
  const allTags = [...new Set([...definedTags.map((t) => t.name), ...Array.from(usedTags)])];
...
async function main() {
  try {
    ...
    console.log("✅ Created placeholder type files. Run API and rebuild to generate actual types.");
  }
}
...
`;
      ...
      groupSchemas.forEach((schemaName) => {
        ...
        const definitionStart = lines.findIndex(
          (line) => line.includes("export interface") || line.includes("export type")
        );
...
async function generateEndpointsIndex() {
  ...
  const endpointExports = endpointTags.map((tag) => `export * from './${tag}';`).join("\n");
