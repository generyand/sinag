...
update_web_env() {
    ...
    # Create or update .env.local with correct API URL
    cat > "$env_file" << EOF
...
EOF
    ...
    # Append any additional non-API settings from existing file
    if [[ -f "${env_file}.bak" ]]; then
        grep -v "^NEXT_PUBLIC_API" "${env_file}.bak" 2>/dev/null | grep -v "^#" | grep -v "^$" >> "$env_file" || true
        rm -f "${env_file}.bak"
    fi
}
