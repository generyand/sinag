#!/usr/bin/env bash
#
# Git Worktree Management Script for SINAG
...
WORKTREE_BASE_DIR="$(dirname "$MAIN_REPO_DIR")/sinag-worktrees"
...
usage() {
    ...
    echo ""
    echo "Commands:"
    echo "  create <name>           Create a new worktree with branch name"
    echo "  create <type> <name>    Create with type prefix (feature/, fix/, etc.)"
    echo ""
    ...
    echo ""
    echo "Examples:"
    echo "  $0 create my-feature                # Branch: my-feature"
    echo "  $0 create feature user-analytics    # Branch: feature/user-analytics"
    echo "  $0 create fix pagination-bug        # Branch: fix/pagination-bug"
    echo "  $0 list"
...
cmd_create() {
    ...
    if [[ -z "$arg1" ]]; then
        print_error "Usage: $0 create <name> [base-branch]"
        print_info "       $0 create <type> <name> [base-branch]"
        print_info ""
        print_info "Examples:"
        print_info "  $0 create my-feature              # Branch: my-feature"
        print_info "  $0 create feature my-feature      # Branch: feature/my-feature"
        exit 1
    fi
    ...
    # Check if first arg is a known type
    case "$arg1" in
        feature|fix|perf|docs|refactor)
            # Type provided: create <type> <name> [base-branch]
            if [[ -z "$arg2" ]]; then
                print_error "Usage: $0 create <type> <name> [base-branch]"
                exit 1
            fi
            branch_name="${arg1}/${arg2}"
            dir_name="${arg1}-${arg2}"
            base_branch="${arg3:-$DEFAULT_BASE_BRANCH}"
            ;;
        *)
            # No type: create <name> [base-branch]
            branch_name="$arg1"
            dir_name="$arg1"
            base_branch="${arg2:-$DEFAULT_BASE_BRANCH}"
            ;;
    esac
...
APIEOF
    ...
    # Create web .env.local with correct ports, preserving non-API settings from main
    local temp_web_file=$(mktemp)
    cat > "$temp_web_file" << WEBEOF
...
WEBEOF
    ...
    # Copy all non-API settings from main web .env.local (preserves Supabase, etc.)
    if [[ -f "$main_web_env" ]]; then
        print_info "Merging settings from main web .env.local (Supabase, etc.)..."
        grep -v "^NEXT_PUBLIC_API" "$main_web_env" 2>/dev/null | grep -v "^#" | grep -v "^$" >> "$temp_web_file" || true
    fi

    mv "$temp_web_file" "$worktree_web_env"
    print_success "Web .env.local configured"

    # Create Docker Compose .env to share the same project name (avoids network conflicts)
    # Also include API_PORT for type generation (orval)
...
COMPOSE_PROJECT_NAME=sinag

# Worktree-specific port configuration
# Used by generate-types.js for Orval type generation
API_PORT=${api_port}
WEB_PORT=${web_port}
DOCKEREOF

    # Create simplified docker-compose.yml for worktrees (avoids network conflicts)
    # NOTE: This is only for worktrees, not the main repo. The main repo has a
    # more complete docker-compose.yml with redis-celery and redis-cache services.
    cat > "${worktree_path}/docker-compose.yml" << 'COMPOSEEOF'
# SINAG Docker Compose Configuration (Worktree)
# Auto-generated by worktree.sh
# Uses host networking to avoid network conflicts with main worktree
...
EOF
    ...
    echo ""
    echo -n "Install dependencies now (pnpm install)? [Y/n] "
    ...
    if [[ ! "$install_reply" =~ ^[Nn]$ ]]; then
        ...
        if pnpm install; then
            ...
            # Also set up Python environment if uv is available
            ...
            print_warning "Failed to install dependencies. Run 'pnpm install' manually."
        fi
    fi
    ...
    echo -e "  ${CYAN}pnpm dev${NC}                 # Starts API:${api_port}, Web:${web_port}"
    echo ""
    echo "Other commands:"
    echo -e "  ${CYAN}pnpm dev:api${NC}             # API only"
    echo -e "  ${CYAN}pnpm dev:web${NC}             # Web only"
    echo -e "  ${CYAN}pnpm dev:no-celery${NC}       # No background tasks"
    echo -e "  ${CYAN}pnpm dev:ports${NC}           # Show port config"
    echo ""
    echo "Or open Claude Code:"
...
sync_env_files() {
    ...
    # Sync Web .env.local (always update with correct ports, preserve other settings)
...
WEBEOF
    ...
    mv "$temp_web_file" "$target_web_env"

    print_success "  Web .env.local synced (API: ${api_port}, Web: ${web_port})"

    # Ensure Docker Compose .env exists with correct ports (for shared Redis and type generation)
...
WEB_PORT=${web_port}
DOCKEREOF
    print_success "  Docker Compose .env synced (ports: API:${api_port}, Web:${web_port})"
}
