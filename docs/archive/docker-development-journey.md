...
This document chronicles the Docker-related issues encountered during SINAG development and how they
were resolved. These are preserved for historical reference and learning purposes.
...
**Problem**: Frontend couldn't fetch data from backend in Docker, though it worked fine when running
manually.
...
- Code was falling back to `http://localhost:8000`, which from inside Docker refers to the Next.js
  container itself, not the API container
...
**Problem**: `curl http://localhost:8000/health` hangs or fails, but
...
**Problem**: API container fails to start with
