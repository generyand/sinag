...
class AnnotationService:
    ...
    def create_annotation(
        self, db: Session, annotation_data: AnnotationCreate, assessor_id: int
    ) -> MOVAnnotation:
        """
        ...
        annotation = MOVAnnotation(
            ...
            rects=[r.model_dump() for r in annotation_data.rects]
    ...
    def get_annotations_for_mov(self, db: Session, mov_file_id: int) -> list[MOVAnnotation]:
        """
        ...
        return (
            ...
            .filter(MOVAnnotation.mov_file_id == mov_file_id)
            .order_by(MOVAnnotation.created_at)
            .all()
        )
    ...
    def get_annotations_for_assessment(
        ...
        return (
            db.query(MOVAnnotation)
            .join(MOVFile, MOVAnnotation.mov_file_id == MOVFile.id)
            .filter(MOVFile.assessment_id == assessment_id)
            .order_by(MOVAnnotation.created_at)
            .all()
        )
    ...
    def delete_annotation(self, db: Session, annotation_id: int, assessor_id: int) -> bool:
        """
