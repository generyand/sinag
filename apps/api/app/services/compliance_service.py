...
class ComplianceService:
    ...
    def _is_checklist_complete(
        ...
        completed_keys_raw = [
            k for k, v in response_data.items()
            if k.startswith("validator_val_") and has_value(v)
        ]

        # Keys where the checkbox/value is True (used for checkbox-only checks)
        true_keys_raw = [
            k for k, v in response_data.items()
            if k.startswith("validator_val_") and v is True
        ]
        ...
        if indicator.mov_checklist_items:
            checklist_items_data = [
                item for item in indicator.mov_checklist_items
        ...
        if validation_rule == "ANY_OPTION_GROUP_REQUIRED":
            ...
            required_items = [
                item for item in checklist_items_data
        ...
        for ga in governance_areas:
            ...
            for parent in parent_indicators:
                ...
                for child in sorted_children:
                    ...
                    is_complete, has_any_checked = self._is_checklist_complete(
                        db, child, response
                    )
                ...
                if parent.is_bbi:
                    ...
                    if all_validated:
                        indicator_data.compliance_status = (
                            "MET" if failed_count == 0 else "UNMET"
                        )
