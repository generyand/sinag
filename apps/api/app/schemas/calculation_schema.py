...
class RuleBase(BaseModel):
    ...
    description: str | None = Field(
        None, max_length=500, description="Human-readable description of the rule"
    )
...
class AndAllRule(RuleBase):
    ...
    conditions: list["CalculationRule"] = Field(
        ..., min_length=2, description="List of rules that must all be true"
    )
...
class OrAnyRule(RuleBase):
    ...
    conditions: list["CalculationRule"] = Field(
        ..., min_length=2, description="List of rules where at least one must be true"
    )
...
class PercentageThresholdRule(RuleBase):
    ...
    threshold: float = Field(..., ge=0, le=100, description="Percentage threshold value (0-100)")
...
class CountThresholdRule(RuleBase):
    ...
    operator: Literal[">=", ">", "<=", "<", "=="] = Field(..., description="Comparison operator")
    threshold: int = Field(..., ge=0, description="Minimum/maximum count of selected checkboxes")
...
class BBIFunctionalityCheckRule(RuleBase):
    ...
    bbi_id: int = Field(..., ge=1, description="ID of the BBI to check (Epic 4 dependency)")
...
class ConditionGroup(BaseModel):
    ...
    rules: list[CalculationRule] = Field(
        ..., min_length=1, description="List of rules in this condition group"
    )
