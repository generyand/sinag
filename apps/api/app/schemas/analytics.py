...
class AffectedBarangay(BaseModel):
    """A barangay affected by an adjustment reason."""
...
class TopReworkReason(BaseModel):
    """A single top reason for adjustment (AI-generated or aggregated)."""
    ...
    source: Literal["adjustment"] = Field(
        default="adjustment",
        description="Source of the reason (unified as 'adjustment')",
    )
...
class TopReworkReasons(BaseModel):
    """Top reasons for adjustment across all assessments (AI-analyzed)."""

    model_config = ConfigDict(from_attributes=True)

    reasons: list[TopReworkReason] = Field(
        default_factory=list,
        description="List of top reasons for adjustment",
        max_length=10,
    )
    total_adjustment_assessments: int = Field(
        ..., description="Total assessments that had adjustments (rework or calibration)", ge=0
    )
