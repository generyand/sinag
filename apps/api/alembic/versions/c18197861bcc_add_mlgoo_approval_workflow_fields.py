...
def upgrade() -> None:
    ...
    op.execute("ALTER TYPE assessment_status_enum ADD VALUE IF NOT EXISTS 'AWAITING_MLGOO_APPROVAL'")

    # Add new notification types
    op.execute("ALTER TYPE notification_type_enum ADD VALUE IF NOT EXISTS 'READY_FOR_MLGOO_APPROVAL'")
    op.execute("ALTER TYPE notification_type_enum ADD VALUE IF NOT EXISTS 'MLGOO_RECALIBRATION_REQUESTED'")
    op.execute("ALTER TYPE notification_type_enum ADD VALUE IF NOT EXISTS 'ASSESSMENT_APPROVED'")
    op.execute("ALTER TYPE notification_type_enum ADD VALUE IF NOT EXISTS 'DEADLINE_EXPIRED_LOCKED'")
    ...
    op.add_column('assessments', sa.Column('is_mlgoo_recalibration', sa.Boolean(), nullable=False, server_default='false'))
    op.add_column('assessments', sa.Column('mlgoo_recalibration_requested_by', sa.Integer(), nullable=True))
    op.add_column('assessments', sa.Column('mlgoo_recalibration_requested_at', sa.DateTime(), nullable=True))
    op.add_column('assessments', sa.Column('mlgoo_recalibration_count', sa.Integer(), nullable=False, server_default='0'))
    op.add_column('assessments', sa.Column('mlgoo_recalibration_indicator_ids', sa.JSON(), nullable=True))
    op.add_column('assessments', sa.Column('mlgoo_recalibration_comments', sa.Text(), nullable=True))

    # Grace Period & Auto-lock tracking
    op.add_column('assessments', sa.Column('grace_period_expires_at', sa.DateTime(), nullable=True))
    op.add_column('assessments', sa.Column('is_locked_for_deadline', sa.Boolean(), nullable=False, server_default='false'))
    ...
    op.create_foreign_key(
        ...
        'assessments', 'users',
        ['mlgoo_approved_by'], ['id'],
        ondelete='SET NULL'
    )
    op.create_foreign_key(
        'fk_assessments_mlgoo_recalibration_requested_by',
        'assessments', 'users',
        ['mlgoo_recalibration_requested_by'], ['id'],
        ondelete='SET NULL'
    )
...
def downgrade() -> None:
    ...
    # Drop foreign keys
    op.drop_constraint('fk_assessments_mlgoo_recalibration_requested_by', 'assessments', type_='foreignkey')
