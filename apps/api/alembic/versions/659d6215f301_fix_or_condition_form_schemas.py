...
def upgrade() -> None:
    ...
    try:
            ...
            for child in indicator_def.children:
                ...
                if form_schema:
                    ...
                    if result.rowcount > 0:
                        ...
                        if child.validation_rule in ['ANY_ITEM_REQUIRED', 'OR_LOGIC_AT_LEAST_1_REQUIRED']:
                            or_indicators_fixed.append(f"{code}: {len(upload_fields)} upload fields")
                        elif child.validation_rule == 'SHARED_PLUS_OR_LOGIC':
                            # Track SHARED+OR indicators separately
                            shared_count = sum(1 for f in upload_fields if f.get('option_group') == 'shared')
                            option_a_count = sum(1 for f in upload_fields if f.get('option_group') == 'option_a')
                            option_b_count = sum(1 for f in upload_fields if f.get('option_group') == 'option_b')
                            or_indicators_fixed.append(f"{code}: SHARED+OR ({shared_count} shared, {option_a_count} opt_a, {option_b_count} opt_b)")
        ...
        if or_indicators_fixed:
            print(f"✅ Fixed OR condition indicators:")
