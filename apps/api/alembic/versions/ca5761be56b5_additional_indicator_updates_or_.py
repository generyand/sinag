...
def upgrade() -> None:
    ...
    if row and row[0]:
        ...
        form_schema["fields"] = [
            ...
            if not (
                f.get("field_id") == "section_header_8"
                or (
                    f.get("field_type") == "section_header"
                    and "Note: Choose either" in f.get("label", "")
                )
            )
        ]
        conn.execute(
            sa.text(
                "UPDATE indicators SET form_schema = CAST(:fs AS json) WHERE indicator_code = '2.1.4'"
            ),
    ...
    if row and row[0]:
        ...
        conn.execute(
            sa.text(
                "UPDATE indicators SET form_schema = CAST(:fs AS json) WHERE indicator_code = '3.3.2'"
            ),
            {"fs": json.dumps(form_schema)},
        )
    ...
    if row and row[0]:
        ...
        if len(form_schema.get("fields", [])) >= 2:
            form_schema["fields"][0]["label"] = (
                "Flow Chart based on Annex C - Establishment of Referral System"
            )
            form_schema["fields"][0]["description"] = (
                "Flow Chart based on Annex C - Establishment of Referral System"
            )
        ...
        conn.execute(
            sa.text(
                "UPDATE indicators SET form_schema = CAST(:fs AS json) WHERE indicator_code = '4.1.7'"
            ),
            {"fs": json.dumps(form_schema)},
        )

    # 4.2.2 - Consolidate to single upload field
    consolidate_to_single_upload(
        conn,
        "4.2.2",
