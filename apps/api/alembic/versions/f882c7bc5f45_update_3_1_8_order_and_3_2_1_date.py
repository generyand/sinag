...
def upgrade() -> None:
    ...
    try:
        ...
        checklist_3_1_8_count = session.query(ChecklistItemModel).filter(
            ChecklistItemModel.item_id == "3_1_8_count"
        ).first()
        if checklist_3_1_8_count:
            checklist_3_1_8_count.display_order = 2
            print("  - Updated 3_1_8_count display_order to 2")

        checklist_3_1_8_b = session.query(ChecklistItemModel).filter(
            ChecklistItemModel.item_id == "3_1_8_b"
        ).first()
        ...
        if indicator_3_2_1:
            ...
            existing_date = session.query(ChecklistItemModel).filter(
