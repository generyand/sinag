...
def upgrade() -> None:
    """Update indicator 1.1.1 checklist items using raw SQL."""
    conn = op.get_bind()
        ...
        conn.execute(
            sa.text("""
                INSERT INTO checklist_items (
                    ...
                    required, requires_document_count, display_order, group_name
                ) VALUES (
                    :indicator_id, :item_id, :label, :item_type, :mov_description,
                    :required, :requires_document_count, :display_order, :group_name
                )
    ...
    if form_schema:
        form_schema_dict = dict(form_schema)
        form_schema_dict["assessor_validation"] = {
            "fields": [
                {
                    "item_id": "1_1_1_bfdp_form",
                    "type": "checkbox",
        ...
        conn.execute(
            ...
            {"id": indicator_id, "schema": json.dumps(form_schema_dict)},
        )
