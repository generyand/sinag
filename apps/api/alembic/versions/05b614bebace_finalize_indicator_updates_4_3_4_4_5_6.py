...
def upgrade() -> None:
    ...
    if result:
        ...
        accomplished = conn.execute(
            sa.text("SELECT display_order FROM checklist_items WHERE indicator_id = :ind_id AND item_id = '4_3_4_physical_accomplished'"),
            {"ind_id": indicator_id}
        ).fetchone()

        reflected = conn.execute(
            sa.text("SELECT display_order FROM checklist_items WHERE indicator_id = :ind_id AND item_id = '4_3_4_physical_reflected'"),
            {"ind_id": indicator_id}
        ).fetchone()
        ...
        if accomplished and reflected:
            ...
            if ref_order < acc_order:
                conn.execute(
                    sa.text("UPDATE checklist_items SET display_order = :order WHERE indicator_id = :ind_id AND item_id = '4_3_4_physical_accomplished'"),
                    {"order": ref_order, "ind_id": indicator_id}
                )
                conn.execute(
                    sa.text("UPDATE checklist_items SET display_order = :order WHERE indicator_id = :ind_id AND item_id = '4_3_4_physical_reflected'"),
                    {"order": acc_order, "ind_id": indicator_id}
                )
                print(f"Swapped 4.3.4 checklist_items display_order: accomplished={ref_order}, reflected={acc_order}")
            else:
...
def downgrade() -> None:
    ...
    conn.execute(
        sa.text("UPDATE indicators SET name = 'Accomplishment Reports: Physical accomplishment OR fund utilization (only 1 of the below reports is required)' WHERE indicator_code = '4.3.4'")
    )
    conn.execute(
        sa.text("UPDATE indicators SET name = 'Accomplishment Reports: Physical accomplishment OR financial utilization (only 1 of the below reports is required)' WHERE indicator_code = '4.5.6'")
    )
