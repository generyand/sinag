...
def upgrade() -> None:
    ...
    conn.execute(
        sa.text(
            "UPDATE indicators SET validation_rule = 'SHARED_PLUS_OR_LOGIC' WHERE indicator_code = '4.8.4'"
        )
    )
    ...
    conn.execute(
        sa.text(
            "UPDATE indicators SET form_schema = CAST(:schema AS jsonb) WHERE indicator_code = '4.8.4'"
        ),
...
def downgrade() -> None:
    ...
    conn.execute(
        sa.text(
            "UPDATE indicators SET validation_rule = 'OR_LOGIC_AT_LEAST_1_REQUIRED' WHERE indicator_code = '4.8.4'"
        )
    )
