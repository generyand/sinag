...
def upgrade() -> None:
    ...
    op.add_column('assessments', sa.Column('is_calibration_rework', sa.Boolean(), nullable=False, server_default='false'))

    # Add calibration_validator_id to track which validator requested calibration
    op.add_column('assessments', sa.Column('calibration_validator_id', sa.Integer(), nullable=True))
