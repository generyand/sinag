...
def upgrade() -> None:
    ...
    op.create_index("idx_assessments_submitted_at", "assessments", ["submitted_at"], unique=False)

    # Index on validated_at - used for validation date queries
    op.create_index("idx_assessments_validated_at", "assessments", ["validated_at"], unique=False)
    ...
    op.create_index("idx_indicators_is_active", "indicators", ["is_active"], unique=False)
