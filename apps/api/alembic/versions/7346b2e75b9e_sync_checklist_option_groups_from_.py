...
OR_LOGIC_INDICATORS = ["2.1.4", "3.1.6", "3.2.3", "4.3.4", "4.5.6", "6.1.4", "6.2.1"]
...
SHARED_PLUS_OR_LOGIC_INDICATORS = ["4.1.6", "4.8.4"]
...
def upgrade() -> None:
    ...
    print(
        f"Updated option_group for {len([k for k, v in OPTION_GROUP_MAPPINGS.items() if v])} checklist items"
    )
    print(f"Updated validation_rule for {len(OR_LOGIC_INDICATORS)} pure OR-logic indicators")
    print(
        f"Updated validation_rule for {len(SHARED_PLUS_OR_LOGIC_INDICATORS)} SHARED + OR-logic indicators"
    )
