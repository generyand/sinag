...
def upgrade() -> None:
    ...
    op.create_index('ix_assessment_cycles_is_active', 'assessment_cycles', ['is_active'], unique=False)
    op.create_index(op.f('ix_assessment_cycles_year'), 'assessment_cycles', ['year'], unique=False)
    op.create_index('ix_assessment_cycles_year_desc', 'assessment_cycles', [sa.literal_column('year DESC')], unique=False)
    op.create_index('uq_assessment_cycles_single_active', 'assessment_cycles', ['is_active'], unique=True, postgresql_where=sa.text('is_active = true'))
    ...
    sa.Column('area_type', sa.Enum('CORE', 'ESSENTIAL', name='area_type_enum', create_constraint=True), nullable=False),
    sa.PrimaryKeyConstraint('id'),
    ...
    op.create_index(op.f('ix_bbis_governance_area_id'), 'bbis', ['governance_area_id'], unique=False)
    ...
    sa.Column('validation_rule', sa.String(length=50), server_default='ALL_ITEMS_REQUIRED', nullable=False),
    ...
    sa.Column('role', sa.Enum('MLGOO_DILG', 'ASSESSOR', 'VALIDATOR', 'BLGU_USER', 'KATUPARAN_CENTER_USER', name='user_role_enum', create_constraint=True), nullable=False),
    sa.Column('validator_area_id', sa.SmallInteger(), nullable=True),
    sa.Column('barangay_id', sa.Integer(), nullable=True),
    sa.Column('preferred_language', sa.String(length=3), nullable=False, comment='Preferred language for AI summaries: ceb (Bisaya), fil (Tagalog), en (English)'),
    ...
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('status', sa.Enum('DRAFT', 'SUBMITTED', 'IN_REVIEW', 'REWORK', 'AWAITING_FINAL_VALIDATION', 'AWAITING_MLGOO_APPROVAL', 'COMPLETED', 'SUBMITTED_FOR_REVIEW', 'VALIDATED', 'NEEDS_REWORK', name='assessment_status_enum', create_constraint=True), nullable=False),
    ...
    sa.Column('final_compliance_status', sa.Enum('PASSED', 'FAILED', name='compliance_status_enum', create_constraint=True), nullable=True),
    ...
    sa.ForeignKeyConstraint(['mlgoo_recalibration_requested_by'], ['users.id'], ondelete='SET NULL'),
    ...
    sa.Column('requires_document_count', sa.Boolean(), server_default='false', nullable=False),
    ...
    op.create_table('deadline_overrides',
    sa.Column('id', sa.Integer(), autoincrement=True, nullable=False),
    ...
    op.create_index(op.f('ix_deadline_overrides_barangay_id'), 'deadline_overrides', ['barangay_id'], unique=False)
    op.create_index('ix_deadline_overrides_barangay_indicator', 'deadline_overrides', ['barangay_id', 'indicator_id'], unique=False)
    op.create_index('ix_deadline_overrides_created_at_desc', 'deadline_overrides', [sa.literal_column('created_at DESC')], unique=False)
    op.create_index(op.f('ix_deadline_overrides_created_by'), 'deadline_overrides', ['created_by'], unique=False)
    op.create_index('ix_deadline_overrides_cycle_barangay', 'deadline_overrides', ['cycle_id', 'barangay_id'], unique=False)
    op.create_index(op.f('ix_deadline_overrides_cycle_id'), 'deadline_overrides', ['cycle_id'], unique=False)
    op.create_index(op.f('ix_deadline_overrides_id'), 'deadline_overrides', ['id'], unique=False)
    op.create_index(op.f('ix_deadline_overrides_indicator_id'), 'deadline_overrides', ['indicator_id'], unique=False)
    op.create_table('indicator_drafts',
    ...
    op.create_index(op.f('ix_indicators_history_indicator_id'), 'indicators_history', ['indicator_id'], unique=False)
    op.create_table('assessment_responses',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('response_data', sa.JSON(), nullable=True),
    sa.Column('is_completed', sa.Boolean(), nullable=False),
    sa.Column('requires_rework', sa.Boolean(), nullable=False),
    sa.Column('validation_status', sa.Enum('PASS', 'FAIL', 'CONDITIONAL', name='validation_status_enum', create_constraint=True), nullable=True),
    ...
    op.create_table('bbi_results',
    sa.Column('id', sa.Integer(), nullable=False),
    sa.Column('status', sa.Enum('HIGHLY_FUNCTIONAL', 'MODERATELY_FUNCTIONAL', 'LOW_FUNCTIONAL', 'FUNCTIONAL', 'NON_FUNCTIONAL', name='bbi_status_enum', create_constraint=True), nullable=False),
    sa.Column('compliance_percentage', sa.Float(), nullable=True, comment='Compliance rate 0-100%'),
    sa.Column('compliance_rating', sa.String(length=30), nullable=True, comment='3-tier rating: HIGHLY_FUNCTIONAL, MODERATELY_FUNCTIONAL, LOW_FUNCTIONAL'),
    sa.Column('sub_indicators_passed', sa.Integer(), nullable=True, comment='Number of sub-indicators that passed'),
    sa.Column('sub_indicators_total', sa.Integer(), nullable=True, comment='Total number of sub-indicators evaluated'),
    sa.Column('sub_indicator_results', sa.JSON(), nullable=True, comment='Detailed pass/fail results for each sub-indicator'),
    ...
    sa.ForeignKeyConstraint(['assessment_id'], ['assessments.id'], ),
    sa.ForeignKeyConstraint(['bbi_id'], ['bbis.id'], ),
    sa.PrimaryKeyConstraint('id')
    )
    op.create_index(op.f('ix_bbi_results_assessment_id'), 'bbi_results', ['assessment_id'], unique=False)
    ...
    sa.Column('notification_type', sa.Enum('NEW_SUBMISSION', 'REWORK_REQUESTED', 'REWORK_RESUBMITTED', 'READY_FOR_VALIDATION', 'CALIBRATION_REQUESTED', 'CALIBRATION_RESUBMITTED', 'VALIDATION_COMPLETED', 'READY_FOR_MLGOO_APPROVAL', 'MLGOO_RECALIBRATION_REQUESTED', 'ASSESSMENT_APPROVED', 'DEADLINE_EXPIRED_LOCKED', 'GRACE_PERIOD_WARNING', name='notification_type_enum', create_constraint=True), nullable=False),
    ...
    op.create_table('mov_annotations',
    sa.Column('id', sa.Integer(), nullable=False),
...
def downgrade() -> None:
    ...
    op.drop_index('uq_assessment_cycles_single_active', table_name='assessment_cycles', postgresql_where=sa.text('is_active = true'))
