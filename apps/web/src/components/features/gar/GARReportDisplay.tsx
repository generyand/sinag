    ...
    <div className="space-y-6">
      ...
      <div className="bg-[var(--card)] border border-[var(--border)] rounded-sm shadow-lg overflow-hidden">
        ...
        {data.governance_areas?.map((area) => (
          <div key={area.area_id} className="border-t border-[var(--border)]">
            {/* Area Header */}
                      ...
                      >
                        <td
                          className={`px-4 py-2 text-sm ${
                            item.item_type === 'info_text' ? 'italic text-[var(--muted-foreground)]' : ''
                          }`}
                          style={{ paddingLeft: '48px' }}
                        >
                          {item.group_name && (
                            <span className="text-xs uppercase text-[var(--muted-foreground)] block mb-1">
                              {item.group_name}
                            </span>
                          )}
                          {item.label}
                        ...
                        <td className="px-4 py-2">
                          {item.item_type !== 'info_text' && item.validation_result && (
                            <div
            ...
            {/* Area Stats */}
            <div className="px-4 py-3 bg-[var(--muted)]/10 flex items-center gap-6 text-sm">
