...
describe('NotificationPanel', () => {
  ...
  describe('Loading State', () => {
    it('shows loading skeletons when loading and no notifications', () => {
      const { container } = render(
        <NotificationPanel {...defaultProps} isLoading={true} />
      );
    ...
    it('shows existing notifications while loading (not skeleton)', () => {
      const notifications = [
        createMockNotification({ id: 1, title: 'Existing Notification' }),
      ];
  ...
  describe('Empty State', () => {
    it('shows empty state message when no notifications', () => {
      ...
      expect(
        screen.getByText(/You'll see notifications here/i)
      ).toBeInTheDocument();
    });
  });
  ...
  describe('Footer', () => {
    it('shows count info when there are more notifications than displayed', () => {
      ...
      expect(
        screen.getByText('Showing 2 of 10 notifications')
      ).toBeInTheDocument();
    });
