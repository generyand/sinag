...
export function AIRecommendationsWidget({ barangayIds }: AIRecommendationsWidgetProps) {
  ...
  const filteredAssessments = useMemo(() => {
    // Cast to typed array inside useMemo to avoid creating new array reference on each render
  ...
  const {
    ...
    error: capdevError,
  } = useGetCapdevAssessmentsAssessmentId(assessmentIdNum, {
    query: {
  ...
  const insightsRaw =
    hasInsights && capdevData.insights ? capdevData.insights[defaultLanguage] : null;
        ...
        <div className="space-y-4">
          <div className="flex items-center space-x-4">
            <div className="flex-1">
              ...
              >
                <SelectTrigger aria-labelledby="assessment-select-label">
                  <SelectValue
                    placeholder={isLoadingAssessments ? "Loading..." : "Choose an assessment..."}
                  />
                </SelectTrigger>
                <SelectContent>
          ...
          {capdevData && !hasInsights && !isLoadingCapdev && (
            <div className="p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg border border-yellow-200 dark:border-yellow-800">
              <p className="text-sm text-yellow-700 dark:text-yellow-300">
                ...
                {capdevData.status === "not_generated" &&
                  "CapDev insights have not been generated yet."}
              </p>
            </div>
          )}
          ...
          {hasInsights && insights && (
            <div className="space-y-6 pt-4">
                    ...
                    <div className="space-y-2">
                      {insights.capacity_development_needs.map((need, index) => {
                        ...
                        const areaOrCategory =
                          "category" in (need as object)
                            ? (need as { category: string }).category
                            : (need as { area: string }).area;
                        const gapOrDesc =
                          "description" in (need as object)
                            ? (need as { description: string }).description
                            : (need as { current_gap: string }).current_gap;
                        return (
                          <div key={index} className="flex items-start space-x-2">
              ...
              <div className="pt-4 border-t border-gray-200 dark:border-gray-700">
                <Button variant="outline" onClick={() => setSelectedAssessmentId("")}>
