...
export function proxy(request: NextRequest) {
  ...
  console.log(
    `Middleware: Path: ${pathname}, Auth token: ${authToken ? "present" : "missing"}, Authenticated: ${isAuthenticated}, Protected: ${isProtectedRoute}`
  );

  // If user is authenticated and trying to access auth routes, redirect to appropriate dashboard
  console.log(
    `Middleware: Checking auth route redirect - Path: ${pathname}, isAuthRoute: ${isAuthRoute}`
  );
  if (isAuthenticated && isAuthRoute) {
    ...
    try {
      ...
      console.log(
        `Middleware: Token decoded successfully. Payload:`,
  ...
  if (isAuthenticated && isProtectedRoute) {
    try {
      ...
      console.log(
        `Middleware: [Route Flags] Admin: ${isAdminRoute}, Assessor: ${isAssessorRoute}, Validator: ${isValidatorRoute}, BLGU: ${isBLGURoute}, Katuparan: ${isKatuparanRoute}`
      );
      ...
      if (
        ...
        console.log(
          `Middleware: IMMEDIATE redirect - ${userRole} user trying to access BLGU route ${pathname}`
        );
        let dashboardUrl;
      ...
      if ((isAdminRoute || isUserManagementRoute) && userRole !== "MLGOO_DILG") {
        console.log(
          `Middleware: Redirecting non-admin user (${userRole}) from ${pathname} to appropriate dashboard`
        );
      ...
      if (isAssessorRoute && userRole !== "ASSESSOR" && userRole !== "MLGOO_DILG") {
        console.log(
          `Middleware: Redirecting non-assessor user (${userRole}) from ${pathname} to appropriate dashboard`
        );
        let dashboardUrl;
      ...
      if (isValidatorRoute && userRole !== "VALIDATOR" && userRole !== "MLGOO_DILG") {
        console.log(
          `Middleware: Redirecting non-validator user (${userRole}) from ${pathname} to appropriate dashboard`
        );
        let dashboardUrl;
        if (userRole === "ASSESSOR") {
          dashboardUrl = new URL("/assessor/submissions", request.url);
      ...
      if (isKatuparanRoute && userRole !== "KATUPARAN_CENTER_USER") {
        console.log(
          `Middleware: Redirecting non-Katuparan user (${userRole}) from ${pathname} to appropriate dashboard`
        );
        let dashboardUrl;
        if (userRole === "MLGOO_DILG") {
          dashboardUrl = new URL("/mlgoo/dashboard", request.url);
      ...
      if (userRole === "KATUPARAN_CENTER_USER" && !isKatuparanRoute) {
        console.log(
          `Middleware: Redirecting Katuparan user (${userRole}) from ${pathname} to Katuparan dashboard`
        );
