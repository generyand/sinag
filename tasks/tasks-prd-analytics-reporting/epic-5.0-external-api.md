...
> **Duration:** 2 weeks
> **Status:** ✅ Completed - 8 stories → 25 atomic tasks
...
---
  ...
  - [ ] **5.4 Story: Rate Limiting Middleware**
    ...
    - [ ] **5.4.2 Atomic:** Create rate limiting dependency for external API
      ...
      - **Acceptance:**
        - Create dependency function `check_external_api_rate_limit(api_key: ExternalAPIKey = Depends(get_current_api_key))` that:
    ...
    - [ ] **5.4.3 Atomic:** Add rate limit headers to responses
      - **Files:** `apps/api/app/core/rate_limiter.py` (extend), `apps/api/app/api/v1/external_api.py` (extend later)
  ...
  - [ ] **5.5 Story: Backend External API Endpoints**
    ...
    - [ ] **5.5.1 Atomic:** Create external API router and endpoint
      ...
      - **Files:** `apps/api/app/api/v1/external_api.py`
      - **Dependencies:** 5.2.4 (service complete), 5.3.2 (auth dependency), 5.4.2 (rate limit dependency)
      - **Acceptance:**
        - Create FastAPI router with prefix `/external` and tag `["external-api"]`
        - Implement `GET /sglgb-data` endpoint
        - Endpoint signature: `get_sglgb_data(cycle_id: Optional[int] = None, start_date: Optional[date] = None, end_date: Optional[date] = None, governance_area: Optional[str] = None, db: Session = Depends(get_db), api_key: ExternalAPIKey = Depends(get_current_api_key), rate_limit_check = Depends(check_external_api_rate_limit))`
  ...
  - [ ] **5.6 Story: API Access Logging**
    ...
    - [ ] **5.6.1 Atomic:** Create access logging service method
      ...
      - **Acceptance:**
        - Add method `log_api_access(db, api_key_id, endpoint, query_params, response_status, ip_address)` that:
    ...
    - [ ] **5.6.2 Atomic:** Integrate logging into external API endpoint
      ...
      - **Acceptance:**
        ...
        - Call `background_tasks.add_task(external_api_service.log_api_access, ...)` after successful response
  ...
  - [ ] **5.8 Story: External API Testing**
    ...
    - [ ] **5.8.2 Atomic:** Write API key authentication and rate limiting tests
      ...
      - **Files:** `apps/api/tests/core/test_auth.py` (extend), `apps/api/tests/core/test_rate_limiter.py` (new)
