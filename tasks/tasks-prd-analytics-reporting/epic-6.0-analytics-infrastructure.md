...
> **PRD Reference:** FR-31 to FR-35 **User Stories:** All (cross-cutting) **Duration:** 1-2 weeks
...
migrations, caching strategy with Redis, query optimization, RBAC enforcement, and timezone handling
to support high-performance analytics features.
...
- [ ] **6.1 Story: Database Migrations for All Analytics Tables**
  ...
  - [ ] **6.1.1 Atomic:** Verify all analytics models are defined
    ...
    - **Acceptance:**
      ...
      - Run
        `python -c "from app.db.models import RecommendationTracking, ExternalAPIKey, APIAccessLog"`
        successfully
  ...
  - [ ] **6.1.2 Atomic:** Create consolidated analytics infrastructure migration
    ...
    - **Acceptance:**
      - Run:
        `cd apps/api && alembic revision --autogenerate -m "add analytics infrastructure tables"`
...
- [ ] **6.2 Story: Database Indexes & Query Optimization**
  ...
  - [ ] **6.2.1 Atomic:** Identify high-frequency query patterns
    - **Files:** `apps/api/app/services/analytics_service.py`,
      `apps/api/app/services/external_api_service.py` (analyze)
...
- [ ] **6.6 Story: Timezone Handling**
  ...
  - **Duration:** 1 day
  - **Dependencies:** None
  - **Files:** `apps/api/app/core/config.py` (extend), `apps/api/app/services/analytics_service.py`
    (extend)
...
- [ ] **6.7 Story: Report Metadata Generation**
  ...
  - [ ] **6.7.3 Atomic:** Display metadata in frontend and PDF exports
    - **Files:** `apps/web/src/components/features/analytics/MetadataDisplay.tsx` (new),
...
- [ ] **6.8 Story: Infrastructure Testing**
  ...
  - [ ] **6.8.2 Atomic:** Write RBAC and timezone tests
    - **Files:** `apps/api/tests/api/test_deps.py` (extend), `apps/api/tests/core/test_timezone.py`
      (new)
  ...
  - [ ] **6.8.3 Atomic:** Write integration tests for caching in analytics
    ...
    - **Acceptance:**
      ...
      - Run with:
        `cd apps/api && pytest tests/services/test_analytics_service.py tests/api/v1/test_analytics.py -vv -k cache`
