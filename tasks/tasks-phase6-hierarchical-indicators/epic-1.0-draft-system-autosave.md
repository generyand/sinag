...
> **PRD Reference:** FR-6.0.2, FR-6.0.7, FR-6.0.8 **User Stories:** US-6.0.2, US-6.0.3 **Duration:**
> 2-3 weeks **Status:** ✅ Complete - All 9 stories implemented and tested
...
**Description:** Implement a hybrid draft system (localStorage + backend) with auto-save
capabilities, optimistic locking for conflict resolution, and cross-device synchronization. This
epic provides the foundational infrastructure for saving incomplete indicator creation work across
multiple sessions.
...
- [x] **1.0 Epic: Draft System & Auto-Save Infrastructure** _(FR-6.0.2, FR-6.0.7, FR-6.0.8)_
  - [x] **1.1 Story: Database Schema for Indicator Drafts**
    - **Scope:** Create SQLAlchemy model and Alembic migration for indicator_drafts table with
    ...
    - **Success Criteria:**
      - `indicator_drafts` table created with fields: `id`, `user_id` (FK), `governance_area_id`
        (FK), `title`, `draft_data` (JSONB), `current_step`, `status`, `version`, `created_at`,
  ...
  - [x] **1.2 Story: Backend Pydantic Schemas for Drafts**
    ...
    - **Success Criteria:**
      - Schema `IndicatorDraftBase` with fields: `title`, `governance_area_id`, `draft_data`,
        `current_step`
      ...
      - Schema `IndicatorDraftSummary` for list view: `id`, `title`, `governance_area_name`,
        `progress_percentage`, `updated_at`, `status`
      - All schemas use proper types and include `Config` with `from_attributes = True`

  - [x] **1.3 Story: Backend Draft Service Layer**
    - **Scope:** Implement business logic for draft CRUD operations, auto-save, and optimistic
      locking
    - **Duration:** 2-3 days
    ...
    - **Success Criteria:**
      ...
      - Optimistic locking: `update_draft()` checks `version` field, raises `ConflictError` if
        version mismatch
  ...
  - [x] **1.5 Story: Type Generation for Draft Endpoints**
    ...
    - **Success Criteria:**
      ...
      - React Query hooks generated: `useCreateIndicatorDraft`, `useGetIndicatorDrafts`,
        `useGetIndicatorDraft`, `useUpdateIndicatorDraft`, `useDeleteIndicatorDraft`
  ...
  - [x] **1.8 Story: Conflict Resolution & Version Control UI**
    ...
    - **Success Criteria:**
      ...
      - If user closes dialog without resolving, keep local changes in localStorage only (don't sync
        to backend)
...
**Epic Status:** 🔄 In Progress - Backend 80% complete (models, schemas, service layer exist),
Frontend foundation ready (state management patterns established)
