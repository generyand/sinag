...
> **PRD Reference:** FR-6.0.5, FR-6.0.6, FR-6.4.x (partial) **User Stories:** US-6.0.7, US-6.0.8,
> US-6.4.1 **Duration:** 3-4 weeks **Status:** 🔄 In Progress - Core validation exists, needs
...
**Description:** Implement comprehensive pre-publish validation, bulk publishing with topological
sorting for parent-child relationships, audit logging, and end-to-end testing across all Phase 6
features. This epic ensures data integrity, referential integrity, and system reliability before
indicators go live.
...
- [ ] **6.0 Epic: Validation, Bulk Publishing & Testing** _(FR-6.0.5, FR-6.0.6, FR-6.4.x)_
  ...
  - [x] **6.2 Story: Backend Schema Completeness Validation** ✅
    ...
    - **Success Criteria:**
      - Add methods to `IndicatorValidationService`:
        ...
        - `validate_calculation_schema(calculation_schema, form_schema)` - Check field references
          exist
      ...
      - **Validation Rules:**
        ...
        - **Remark Schema:** Template variables match available variables (barangay_name, score,
          form fields)
  ...
  - [x] **6.4 Story: Backend Topological Sorting for Bulk Publish** ✅
    ...
    - **Success Criteria:**
      ...
      - **Topological Sort Algorithm:**
        ...
        - Use Kahn's algorithm: start with nodes with no parents (roots), process children after
          parents
  ...
  - [x] **6.5 Story: Backend Audit Logging for Publications** ✅
    ...
    - **Success Criteria:**
      - **AuditLog Model** (if not exists):
        - Fields: `id`, `user_id`, `action`, `entity_type`, `entity_id`, `details` (JSONB),
          `timestamp`
      ...
      - **Integration:**
        ...
        - Details JSONB includes:
          `{"draft_id": ..., "indicator_count": ..., "governance_area_id": ...}`
  ...
  - [x] **6.6 Story: Frontend Pre-Publish Validation Summary** ✅
    ...
    - **Files:**
      - `apps/web/src/components/features/indicators/builder/ValidationSummary.tsx` (already
        exists!)
    ...
    - **Success Criteria:**
      - `ValidationSummary` component:
        ...
        - Displays validation results in expandable sections:
          ...
          2. **Schemas** - Group errors by indicator and schema type (Form, MOV, Calculation,
             Remark)
      ...
      - `PublishDialog` component:
        ...
        - Confirmation message: "You are about to publish [N] indicators. This action cannot be
          undone."
  ...
  - [x] **6.8 Story: Integration Testing for Full Workflow** ✅
    ...
    - **Files:**
      ...
      - `apps/web/src/__tests__/integration/indicator-builder-workflow.test.tsx` (optional - E2E
        recommended instead)
    ...
    - **Success Criteria:**
      - **Backend Integration Tests** (`test_indicator_workflow.py`) ✅ **COMPLETE**:
        ...
        - ✅ Test 3: Topological sort (create parent after child in draft → publish succeeds in
          correct order)
  ...
  - [x] **6.9 Story: Spec v1.4 Validation Testing** ✅
    ...
    - **Success Criteria:**
      ...
      **Note:** Rather than creating JSON fixtures for all 29 individual indicators, we validated
      that our system can handle all the PATTERNS used across the 29 indicators (hierarchical
      structures, BBIs, date validation, mutually exclusive scenarios, etc.). This approach ensures
      the validation system works for any indicator configuration following Spec v1.4 patterns.
  ...
  - [x] **6.10 Story: Comprehensive Test Suite & Coverage Report** ✅
    ...
    - **Success Criteria:**
      - **Backend Test Coverage:** ✅ **COMPLETE**
        - ✅ Run:
          `pytest tests/integration/ tests/services/ tests/test_spec_v1_4_validation.py --cov=app --cov-report=html --cov-report=term`
...
**Epic Status:** ✅ **COMPLETE** - All 10 stories finished, 53 tests passing, validation system
production-ready

**Phase 6 Completion:** Epic 6.0 COMPLETE - Validation & Testing infrastructure ready for production
deployment
...
**Ready for Phase 3 (Atomic Tasks):** Awaiting user "Go" to expand each story into atomic tasks (2-8
hours each)
